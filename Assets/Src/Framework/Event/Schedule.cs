...
namespace Framework
{
    namespace Event
    {
		public abstract class ScheduleEvent
        {
            ...
            public bool Update(float delta)
            {
                if (!m_pause)
                {
                    ...
                    // 是否达到更新时间
                    else if ((m_delayTime == 0 || m_runTime > 0) && m_realTime >= m_runTime + m_updateTime)
                    {
		...
		public class Schedule : Singleton.Singleton<Schedule>
        {
            ...
            /// 时刻表
            /// </summary>
            List<ScheduleEvent> m_schedule;

            /// <summary>
            /// 将要添加的时刻事件表
            /// </summary>
            List<ScheduleEvent> m_willAdd;

            /// <summary>
            /// 将要移除的时刻事件表
            /// </summary>
            List<ScheduleEvent> m_willRemove;
